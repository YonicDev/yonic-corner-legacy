...
None of the browsers I've targeted this blog to **won't work under the latest versions of encryption protocols** used by nearly all websites since a few years ago (TLS 1.1 and higher). This has forced me to host the blog in a place where HTTPS is not forced, which meant that many web hosting solutions I'm used like Netlify got benched.

The only reasonable place to do it for now without setting up a server on my own is **GitHub Pages**. I don't really mind having a fully static blog, but having SSR might be used to leverage the rendering of certain dynamic elements, like the site marquee.

Right now, I'm using the default domain, so I can't turn off HTTPS for now. This is definitely bound to change soon so that it removes this final obstacle.