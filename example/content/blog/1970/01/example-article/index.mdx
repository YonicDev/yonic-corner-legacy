...
* `singleLine`: A boolean flag that determines if the bubble has only one line. Instead of rendering `<CharaName>Content</CharaName>`, it will render `CharaName: Content`.
